    ...
    <li><a href="report.php"><i class="fa fa-bar-chart" aria-hidden="true"></i> <span> Reports</span></a></li>
        ...
        <div class="content-wrapper">
            <section class="content-header">
                <h1>
                    Card SME Bank Molino Branch Member Management
                    <small></small>
                </h1>
                    ...
                    <div class="col-lg-12">
                        <table width="100%" class="table table-striped table-bordered table-hover" id="prodTable">
                            ...
                            <tbody>
                               ...
                               <!-- lookm mo yung code yan yung concatisht yung pangalan para isang tawag nlng tpos where where nya tignan mo din ok2
                                $result=mysqli_query($conn,"SELECT UID, Concat(lname, ', ', fname, ' ', mname) as 'Fullname', gender, bday, email, FROM tblusers WHERE account_type='1'");                       
                                
					$pq=mysqli_query($conn,"SELECT * FROM tblaccounts LEFT JOIN tblloanapplication ON tblloanapplication.MID = tblaccounts.AID LEFT JOIN tblmember ON tblmember.AID = tblaccounts.AID WHERE tblaccounts.AID = tblmember.AID");-->  
                               
                                <?php
                $one=1;
					$pq=mysqli_query($conn,"SELECT * FROM tblaccounts WHERE account_type ='$one' ");
					...
					?>
                                    <tr>
                                        ...
                                        <td>
                                           <?php echo $pqrow['datecreated']; ?>
                                        </td>

                                        <td>
                                            <?php echo $pqrow['account_lvl']; ?>
                                        </td>
                                        <td>
                                            <button class="btn btn-success btn-sm" data-toggle="modal" data-target="#promoteuser_<?php echo $pid; ?>"><i class="glyphicon glyphicon-open"></i> Promote</button>
                                            <button class="btn btn-danger btn-sm" data-toggle="modal" data-target="#demoteuser_<?php echo $pid; ?>"><i class="fa fa-trash"></i> Demote</button>
